	...
	function init(plot) {
		...
		function combine(data) {
			...
			for (var i = 0; i < data.length; ++i) {
				...
				// than creating a
				// new one; this is more efficient and preserves
				// any extra data
			...
			// Count the number of slices with percentages below the
			// combine
		...
		function draw(plot, newCtx) {
			...
			// we need to
			// add a new series. Since Flot gives plugins no way to
			// modify the
			...
			// The plugin-global 'processed' flag is used to control
			// this hack;
			...
			// no-ops; they
			// call processDatapoints, the flag is true, and nothing
			// happens.
			...
			// slice-combine.
			// This is really a hack; in 0.9 we need to give plugins
			// a proper
			...
			// Keep shrinking the pie's radius until drawPie returns
			// true,
			...
			function drawPie() {
				...
				function drawSlice(angle, color, fill) {
					...
					// false); // This doesn't work properly
					// in Opera
		...
		function drawDonutHole(layer) {
			if (options.series.pie.innerRadius > 0) {
				...
				layer.globalCompositeOperation = "destination-out"; // this
										    // does
										    // not
										    // work
										    // with
		...
		function findNearbySlice(mouseX, mouseY) {
			...
			for (var i = 0; i < slices.length; ++i) {
				...
				if (s.pie.show) {
					...
					// ctx.scale(1,
					// options.series.pie.tilt); // this
					// actually seems to break everything
					// when here.
					...
					if (ctx.isPointInPath) {
						...
						// mathmatical trickery here
						// with the Y-coordinate to
						// compensate for pie tilt?
		...
		series: {
			pie: {
				...
				radius: "auto",	// actual radius of the visible
						// pie (based on full calculated
						// radius if <=1, or hard pixel
						// value)
				...
				combine: {
					threshold: -1,	// percentage at which
							...
							// slices into one
							// larger slice
					color: null,	// color to give the new
							// slice (auto-generated
							// if null)
					label: "Other"	// label to give the new
							// slice
				},
				highlight: {
					// color: "#fff", // will add this
					// functionality once parseColor is
					// available
					opacity: 0.5
				}
			}
