...
(function($) {
	...
	function Canvas(cls, container) {
		...
		if (element == null) {
			...
			// If HTML5 Canvas isn't available, fall back to
			// [Ex|Flash]canvas
		...
		// Determine the screen's ratio of physical to
		// device-independent
		// pixels. This is the ratio between the canvas width that the
		// browser
		// advertises and the number of pixels actually present in that
		// space.

		// The iPhone 4, for example, has a device-independent width of
		// 320px,
		// but its screen is actually 640px wide. It therefore has a
		// pixel
		...
		// Size the canvas to match the internal dimensions of its
		// container

		this.resize(container.width(), container.height());

		// Collection of HTML div layers for text overlaid onto the
		// canvas

		this.textContainer = null;
		this.text = {};

		// Cache of text fragments and metrics, so we can avoid
		// expensively
	...
	Canvas.prototype.resize = function(width, height) {
		...
		// Resize the canvas, increasing its density based on the
		// display's
		// pixel ratio; basically giving it more pixels without
		// increasing the
		// size of its element, to take advantage of the fact that
		// retina
		// displays have that many more pixels in the same advertised
		// space.

		// Resizing should reset the state (excanvas seems to be buggy
		// though)
		...
		// The
		// restore ensure that we're really back at the initial state,
		// and
		// should be safe even if we haven't saved the initial state
		// yet.

		context.restore();
		context.save();

		// Scale the coordinate space to match the display density; so
		// even though we
		// may have twice as many pixels, we still want lines and other
		// drawing to
	...
	Canvas.prototype.render = function() {
		...
		// For each text layer, add elements marked as active that
		// haven't
		// already been rendered, and remove those that are no longer
		// active.
	...
	// }
	//
	// Each position after the first receives a clone of the original
	// element.
	//
	// The idea is that that the width, height, and general 'identity' of
	// the
	// text is constant no matter where it is placed; the placements are a
	// secondary property.
	//
	// Canvas maintains a cache of recently-used text info objects;
	// getTextInfo
	// either returns the cached element or creates a new entry.
	//
	// @param {string} layer A string of space-separated CSS classes
	// uniquely
	...
	Canvas.prototype.getTextInfo = function(layer, text, font, angle, width) {
		...
		// Cast the value to a string, in case we were given a number or
		// such

		text = "" + text;

		// If the font is a font-spec object, generate a CSS font
		// definition
		...
		// Retrieve (or create) the cache for the text's layer and
		// styles
		...
		// If we can't find a matching element in our cache, create a
		// new one
	...
	//
	// The text isn't drawn immediately; it is marked as rendering, which
	// will
	...
	// Angle is currently unused, it will be implemented in the future.
	// @param {number=} width Maximum width of the text before it wraps.
	// @param {string=} halign Horizontal alignment of the text; either
	// "left",
	// "center" or "right".
	...
	Canvas.prototype.addText = function(layer, x, y, text, font, angle, width, halign, valign) {
		...
		// If the text doesn't exist at this position, create a new
		// entry

		// For the very first position we'll re-use the original
		// element,
	...
	// add back most or all of it later, as when redrawing axes, for
	// example.
	//
	// @param {string} layer A string of space-separated CSS classes
	// uniquely
	...
	// @param {(string|object)=} font Either a string of space-separated CSS
	// classes or a font-spec object, defining the text's font and style.
	// @param {number=} angle Angle at which the text is rotated, in
	// degrees.
	// Angle is currently unused, it will be implemented in the future.
	    ...
	    // plug-in,
            ...
            if (!axis.tickGenerator) {
				...
				axis.tickFormatter = function (value, axis) {
					...
					// If tickDecimals was specified, ensure
					// that we have exactly that
					// much precision; otherwise default to
